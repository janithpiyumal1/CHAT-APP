...
const ChatContainer = ({setSelctedUser,selectedUser}) => {
  ...
  return selectedUser ? (
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messagesDummyData.map((msg, index) => (
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'}`}>
            {msg.image ?( 
              <img src={msg.image} alt="" className='max-w-[230px] border border-stone-600 rounded-lg overflow-hidden mb-8' />
            ) : (
            ...
            <div className='text-center text-xs'>
              <img src={msg.senderId === '680f50e4f10f3cd28382ecf9' 
                ...
                <p className="text-sm text-white">{formatMessageTime(msg.createdAt)}</p>
            </div>
          </div>
        ))}
        <div ref={scrollEnd}>
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
      ...
      <p className='text-white text-lg font-medium'>Chat Anytime</p>
    </div>
  )
}
